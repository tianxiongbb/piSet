...
fun_plot_lendis2=function(l1, l2, rn, m){
	...
	if(rn!=0){
		...
		for(sn in names(l1)){
			par(mar=c(0,2,2,0.5))
			barplot(l1[[sn]][,rn],space=0,border="white",col="#e41a1c",xaxt="n")
			if(sn==names(l1)[1]){mtext(m,3,font=2)}
			par(mar=c(2,2,0,0.5))
			barplot(-l2[[sn]][,rn],space=0,border="white",col="#377eb8",xaxt="n")
			axis(1,1:21-0.5,label=15:35,lwd=0,padj=-0.3)
		}
	}else{
		...
		for(sn in names(l1)){
			par(mar=c(0,2,2,0.5))
			barplot(apply(l1[[sn]],1,sum),space=0,border="white",col="#e41a1c",xaxt="n")
			mtext(m,3,font=2)
			par(mar=c(2,2,0,0.5))
			barplot(-apply(l2[[sn]],1,sum),space=0,border="white",col="#377eb8",xaxt="n")
			axis(1,1:21-0.5,label=15:35,lwd=0,padj=-0.3)
		}
	}
}
fun_plot_pp2=function(l, rn){
	...
	if(rn!=0){
		...
		for(sn in names(l)){
			par(mar=c(3,3,3,1),cex=0.65)
			barplot(l[[sn]][,rn],space=0,border="white",col="black",xaxt="n")
			axis(1,c(1,10,20,30)-0.5,label=c(1,10,20,30),lwd=0)
			if(sn==names(l)[1]){mtext("ping-pong",3,font=2)}
			zs=round((l[[sn]][10,rn]-mean(l[[sn]][c(1:9,11:30),rn]))/sd(l[[sn]][c(1:9,11:30),rn]),2)
			text(10,max(l[[sn]][,rn])*9/10,label=paste("z-score=",zs,sep=""),col="#e41a1c",font=2,pos=4,cex=1.2)
		}
	}else{
		for(sn in names(l)){maxN=max(maxN, max(apply(l[[sn]],1,sum)))}
		for(sn in names(l)){
			par(mar=c(3,3,3,1),cex=0.65)
			barplot(apply(l[[sn]],1,sum),space=0,border="white",col="black",xaxt="n")
			axis(1,c(1,10,20,30)-0.5,label=c(1,10,20,30),lwd=0)
			if(sn==names(l)[1]){mtext("ping-pong",3,font=2)}
			zs=round((apply(l[[sn]],1,sum)[10]-mean(apply(l[[sn]],1,sum)[c(1:9,11:30)]))/sd(apply(l[[sn]],1,sum)[c(1:9,11:30)]),2)
			text(10,max(apply(l[[sn]],1,sum))*9/10,label=paste("z-score=",zs,sep=""),col="#e41a1c",font=2,pos=4,cex=1.2)
		}
	}
}
fun_plot_bucket2=function(l, rn){
	...
	for(sn in names(l)){
		x=l[[sn]][which(l[[sn]][,1]==rn),]
		par(mar=c(0,2,2,0.5))
		barplot(as.vector(x[,3]),col="#fcbba1",border="#fcbba1",xaxt="n",space=0,yaxt="n")
		barplot(as.vector(x[,5]),col="#a50f15",border="#a50f15",xaxt="n",space=0,yaxt="n",add=T)
		h=signif(max(x[,3])/4,1)
		axis(2,h*(0:4),label=h*(0:4))
		mtext(sn,3,font=2)
		par(mar=c(2,2,0,0.5))
		barplot(as.vector(x[,4]),col="#9ecae1",border="#9ecae1",xaxt="n",space=0,yaxt="n")
		barplot(as.vector(x[,6]),col="#08519c",border="#08519c",xaxt="n",space=0,yaxt="n",add=T)
		h=signif(min(x[,4])/4,1)
		axis(2,h*(0:4),label=c("",h*(1:4)))
		axis(1,c(1,floor(dim(x)[1]/4*(1:4)))-0.5,label=c(1,x[dim(x)[1]/4*(1:4),2]))
	}
}
