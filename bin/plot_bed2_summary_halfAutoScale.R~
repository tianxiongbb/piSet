...
fun_plot_lendis2=function(l1, l2, rn, m){
	if(rn!=0){
		for(sn in names(l1)){
			maxN=max(max(l1[[sn]][,rn]), max(l2[[sn]][,rn]))
			par(mar=c(0,2,2,0.5))
		...
		for(sn in names(l1)){
			maxN=max(max(apply(l1[[sn]],1,sum)), max(apply(l2[[sn]],1,sum)))
			par(mar=c(0,2,2,0.5))
...
fun_plot_bucket2=function(l, rn){
	for(sn in names(l)){
		...
		maxN=max(x[,3], -x[,4])
		par(mar=c(0,2,2,0.5))
		barplot(as.vector(x[,3]),col="#a50f15",border="#a50f15",xaxt="n",space=0,ylim=c(0,maxN))
		barplot(as.vector(x[,5]),col="#fcbba1",border="#fcbba1",xaxt="n",space=0,yaxt="n",add=T)
		mtext(sn,3,font=2)
		par(mar=c(2,2,0,0.5))
