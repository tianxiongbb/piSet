 ...
 * Authors: Peter Meric
 *
 * File Description:  NetSchedule grid client for TrackManager remove-user-track request/reply
 *
 */

/// @file tmgr_removeusertrack_client.hpp
/// NetSchedule grid client for TrackManager Remove User Track request/reply
...
class CTMS_RemoveUserTrack_Client : private CGridRPCBaseClient<CAsnBinCompressed>
{
private:
    typedef CGridRPCBaseClient<ncbi::CAsnBinCompressed> TBaseClient;

public:
    typedef objects::CTMgr_RemoveUserTrackRequest TRequest;
    typedef objects::CTMgr_RemoveUserTrackReply TReply;
    typedef CConstRef<TRequest> TRequestCRef;
    typedef CRef<TReply> TReplyRef;


public:
    CTMS_RemoveUserTrack_Client(const string& NS_service,
                                const string& NS_queue,
                                const string& client_name,
                                const string& NC_registry_section
                               );

    CTMS_RemoveUserTrack_Client(const string& NS_registry_section = "netschedule_api",
                                const string& NC_registry_section = kEmptyStr
                               );

    virtual ~CTMS_RemoveUserTrack_Client();
