...
{
    if ( !m_Search ) {
        m_Search.Reset(new TSearch());
        return;
    }
    (*m_Search).Reset();
}

void CIteration_Base::SetSearch(CIteration_Base::TSearch& value)
{
    m_Search.Reset(&value);
}
...
BEGIN_NAMED_BASE_CLASS_INFO("Iteration", CIteration)
{
    ...
    ADD_NAMED_REF_MEMBER("search", m_Search, CSearch);
    info->RandomOrder();
